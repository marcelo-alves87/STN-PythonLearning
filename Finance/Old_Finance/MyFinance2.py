...
df_volume = df['Volume'].resample('B').sum()
df['RSI'] = RSI(df)
df_rsi = df['Volume'].resample('B').last()
df_rsi.fillna(0,inplace=True)
...
ax2.fill_between(df_rsi.index.map(mdates.date2num), df_rsi.values, 0)
#RSI(df, ax2)
plt.title(ticker)
plt.show()
